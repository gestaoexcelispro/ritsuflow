...
export default function RitsuCadRibbonBridge() {
  ...
  const smartTakeoffSection = toolbar
    ? createPortal(
        <div ref={smartMenuRef} data-ritsucad-smart-takeoff="true" style={{ position: 'relative', flex: '0 0 auto', display: 'flex', alignItems: 'flex-end', gap: 3, padding: '15px 7px 1px 12px', marginLeft: 5, borderLeft: '1px solid #d5e0e8' }}>
          ...
          {smartGroups.map((group) => {
            ...
            return (
              <div key={group.name} style={{ position: 'relative' }}>
                <CategoryButton
                  label={group.name === 'Architectural' ? 'Architecture' : group.name}
                  icon={groupIcon(group.name)}
                  disabled={!hasProjectDrawing}
                  active={Boolean(selected)}
                  onClick={() => hasProjectDrawing && setOpenGroup((current) => current === group.name ? null : group.name)}
                />
                  ...
                  <div style={{ position: 'absolute', zIndex: 1000, top: 45, left: 0, minWidth: 170, padding: 5, border: '1px solid #bfd0da', borderRadius: 7, background: '#fff', boxShadow: '0 8px 24px rgba(15,54,75,.18)' }}>
                    {group.items.map((item) => (
                      <button key={item.id} type="button" onClick={() => activateSmartTakeoff(item)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '7px 9px', border: 0, borderRadius: 5, background: activeSmartTakeoff === item.id ? '#e8faf8' : '#fff', color: activeSmartTakeoff === item.id ? '#007f79' : '#173e52', font: 'inherit', fontSize: 11, fontWeight: 800, textAlign: 'left', cursor: 'pointer' }}>
                        <span style={{ width: 18, textAlign: 'center', fontSize: 14 }}>{smartIcon(item.id)}</span>
                        <span style={{ flex: 1 }}>{item.label}</span>
                        <span style={{ color: '#78909c', fontSize: 9 }}>{item.measurementTool}</span>
                      </button>
                    ))}
                  </div>
                ) : null}
  ...
  const projectDataSection = toolbar && hasProjectDrawing
    ? createPortal(
        <div data-ritsucad-project-data="true" style={{ position: 'relative', flex: '0 0 auto', display: 'flex', alignItems: 'flex-end', gap: 2, padding: '15px 5px 1px 12px', marginLeft: 5, borderLeft: '1px solid #d5e0e8' }}>
