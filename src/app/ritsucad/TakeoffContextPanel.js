...
export default function TakeoffContextPanel() {
  ...
  return (
    <aside style={panel}>
      <div style={header}>
        <div>
          <div style={eyebrow}>PROJECT TAKEOFF</div>
          <div style={title}>Takeoff Context</div>
        </div>
        <span style={ready ? readyBadge : setupBadge}>{ready ? 'Ready' : 'Setup'}</span>
      </div>
      ...
      <div style={body}>
        {loading ? <div style={helper}>Loading project locations and scope…</div> : (
          <>
            ...
            <select value={locationId} onChange={(event) => setLocationId(event.target.value)} style={control}>
              <option value="">Select LBS location…</option>
              {locationRows.map((location) => (
                <option key={location.id} value={location.id}>{'— '.repeat(location.depth)}{location.name}</option>
              ))}
            </select>

            <label style={label}>Scope activity</label>
            <select value={serviceId} onChange={(event) => setServiceId(event.target.value)} style={control}>
              <option value="">Select scope activity…</option>
              {services.map((service) => (
                <option key={service.id} value={service.id}>{service.code ? `${service.code} · ` : ''}{service.name}{service.unit ? ` · ${service.unit}` : ''}</option>
              ))}
            </select>
            ...
            {ready ? (
              <div style={summary}>
                ...
                <div style={summaryNote}>Choose a RitsuCAD takeoff tool and measure the drawing. The calculated quantity will inherit this context.</div>
              </div>
            ) : (
              <div style={helper}>Select where the work occurs and what scope is being quantified before starting a project takeoff.</div>
            )}
