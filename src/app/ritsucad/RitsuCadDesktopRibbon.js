...
export default function RitsuCadDesktopRibbon(){
  ...
  useEffect(()=>{
    ...
    const find=()=>{
      ...
      let node=header?.parentElement||null, root=null
      while(node){
  ...
  return createPortal(<>
    ...
    <section className="rfDesktopRibbon" data-ritsucad-desktop-ribbon="true">
      <nav className="rfRibbonTabs">{TABS.map(t=><button key={t} type="button" className={tab===t?'active':''} onClick={()=>setTab(t)}>{t}</button>)}</nav>
...
.rfDesktopRibbon{position:fixed!important;z-index:95!important;top:72px!important;height:126px!important;display:flex!important;flex-direction:column!important;background:#eef3f6;border-bottom:1px solid #9eafb9;color:#123b50;overflow:hidden}
.rfRibbonTabs{height:34px;flex:0 0 34px;display:flex;align-items:flex-end;gap:2px;padding:0 18px;border-bottom:1px solid #c9d5dc;background:#fff;overflow:hidden}.rfRibbonTabs button{height:34px;min-width:78px;padding:0 16px;border:0;border-bottom:3px solid transparent;background:transparent;color:#244b60;font:inherit;font-size:11px;font-weight:850;cursor:pointer}.rfRibbonTabs button:hover{background:#e9f1f4}.rfRibbonTabs button.active{color:#008f88;border-bottom-color:#00a59d;background:#f4fbfb}
.rfRibbonCommands{height:92px;flex:0 0 92px;display:flex;align-items:stretch;padding:4px 12px 3px;overflow-x:auto;overflow-y:hidden;scrollbar-width:none}.rfRibbonCommands::-webkit-scrollbar{display:none}.rfCommandGroup{display:flex;flex:0 0 auto;flex-direction:column;align-items:stretch;padding:0 6px;border-right:1px solid #bdcbd3}.rfCommandGroup:first-child{padding-left:0}.rfCommandGroup:last-child{border-right:0}.rfCommandItems{display:flex;flex:1;align-items:stretch}.rfCommandGroup small{text-align:center;color:#607986;font-size:9px;line-height:14px;white-space:nowrap}.rfCommandItems button{min-width:72px;height:70px;padding:5px 8px;border:0;background:transparent;color:#173f52;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;font:inherit;cursor:pointer}.rfCommandItems button:hover{background:#dcebed}.rfCommandItems button:disabled{opacity:.35}.rfCommandItems button>span{height:28px;font-size:24px;line-height:28px;color:#075f78}.rfCommandItems button>strong{font-size:9px;white-space:nowrap}
.rfDrawingTabs{position:fixed!important;z-index:94!important;top:198px!important;height:34px!important;display:flex;align-items:flex-end;padding:0 12px;background:#e8eef2;border-bottom:1px solid #b7c6ce}.rfDrawingTabs button{height:30px;padding:0 14px;border:1px solid transparent;background:transparent;color:#254a5d;font:inherit;font-size:10px;font-weight:750}.rfDrawingTabs button.active{background:#fff;border-color:#b7c6ce;border-bottom-color:#fff}.rfDrawingTabs span{margin-left:16px;color:#7c909b}
