...
function getNextStatus(status: string) {
  const index = SESSION_STATUSES.indexOf(status)
...
function getPreviousStatus(status: string) {
  const index = SESSION_STATUSES.indexOf(status)
...
function getContrastYIQ(hexColor?: string | null) {
  const clean = String(hexColor || '#64748b')
    .replace('#', '')
    .padEnd(6, '0')
    .slice(0, 6)
  ...
  return yiq >= 145 ? '#071c31' : '#ffffff'
}


function buildLocationOptions(locations: any[]) {
  const map = new Map(
  ...
  locations.forEach((location) => {
    ...
    childrenCount.set(
      location.parent_id,
      (childrenCount.get(location.parent_id) || 0) + 1,
    )
  })

  const buildPath = (location: any) => {
    if (!location) {
      return ''
    }
  ...
  const leaves = locations.filter(
    (location) =>
      !childrenCount.has(location.id),
  )
...
export default function PullPlanningPage() {
  ...
  const [projectCoverUrls, setProjectCoverUrls] =
  ...
  const [workPackages, setWorkPackages] =
    useState<any[]>([])
  ...
  const [horizonStart, setHorizonStart] =
    useState('')

  const [horizonEnd, setHorizonEnd] =
    useState('')
  ...
  const [activityMode, setActivityMode] =
    useState<'milestone' | 'predecessor'>(
      'milestone',
    )
  ...
  const [activityUnit, setActivityUnit] =
    useState('')

  const [activityCrew, setActivityCrew] =
    useState('')
  ...
  const [activityRate, setActivityRate] =
    useState('')
  ...
  useEffect(() => {
    pullItemsRef.current = pullItems
  }, [pullItems])
  ...
  const selectedMilestones =
    selectedSessionId
      ? milestonesBySession[selectedSessionId] || []
      : []
  ...
  // ==========================================================
  // CREATE ACTIVITY + PULL ITEM + OPTIONAL HANDOFF
  // ==========================================================
