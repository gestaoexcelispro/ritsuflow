 ...
 return <main className={styles.shell}>
  ...
  <section className={styles.workspace}>
   ...
   <div className={styles.formArea}>
    <div className={styles.heading}><span>STEP {step} OF 5</span><h1>{steps[step-1][1]}</h1><p>{step===1?'Create the single project record shared by the RitsuFlow modules.':step===2?'Register the project location. Address fields can later be connected to ZIP-code lookup.':step===3?'Define contract value, material composition and retainage conditions.':step===4?'Define the contractual timeline and what successful delivery means to the client.':'Review the project before creating the shared record.'}</p></div>
    <div className={styles.card}>
     ...
     {step===2&&<div className={styles.grid}><label>ZIP Code<input value={form.postal_code} onChange={e=>set('postal_code',e.target.value)} placeholder="ZIP / Postal Code"/></label><label>Country<input value={form.country_code} onChange={e=>set('country_code',e.target.value)} placeholder="BR"/></label><label className={styles.wide}>Address<input value={form.address_line} onChange={e=>set('address_line',e.target.value)} placeholder="Automatically filled through ZIP code"/></label><label>Number<input value={form.address_number} onChange={e=>set('address_number',e.target.value)} placeholder="Building / street number"/></label><label>Neighborhood<input value={form.neighborhood} onChange={e=>set('neighborhood',e.target.value)} placeholder="Automatically filled when available"/></label><label>City<input value={form.city} onChange={e=>set('city',e.target.value)} placeholder="Automatically filled"/></label><label>State<input value={form.state_region} onChange={e=>set('state_region',e.target.value)} placeholder="Automatically filled"/></label><div className={styles.note}><b>ZIP-code lookup</b><span>Address, neighborhood, city, state and country remain editable because postal databases may return incomplete information.</span></div></div>}
