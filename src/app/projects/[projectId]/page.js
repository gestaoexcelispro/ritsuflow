...
export default function ProjectDetailPage() {
  ...
  async function deleteProject() {
    ...
    const label = project.project_id ? `${project.project_id} — ${project.name}` : project.name
    const confirmed = window.confirm(`Delete project "${label}"?\n\nThis action cannot be undone.`)
    if (!confirmed) return
  ...
  return (
    <main style={shell}>
      <header style={header}>
        ...
        <div style={{ flex: 1 }}><div style={{ fontSize: 22, fontWeight: 800 }}>Project</div><div style={{ fontSize: 11, opacity: .76 }}>Shared project record</div></div>
      ...
      <section style={content}>
        ...
        {!loading && error && !project && <div style={panel}><h2 style={{ marginTop: 0 }}>Unable to open project</h2><p>{error}</p><Link href="/projects" style={{ color: '#087f8c', fontWeight: 700 }}>Return to Projects →</Link></div>}
        ...
        {!loading && project && <>
          <section style={overview}>
            <div style={{ minWidth: 0 }}>
              ...
              <div style={identityLine}>
                <h1 style={projectName}>{project.name || 'Untitled Project'}</h1><span style={identityDot}>•</span>
                <strong style={projectNumber}>{project.project_id || 'Project ID pending'}</strong><span style={identityDot}>•</span>
                <span>{project.client_name || 'Client not defined'}</span><span style={identityDot}>•</span>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <button type="button" onClick={deleteProject} disabled={deleting} style={{ ...deleteButton, opacity: deleting ? .55 : 1, cursor: deleting ? 'not-allowed' : 'pointer' }}>{deleting ? 'Deleting...' : 'Delete Project'}</button>
            </div>
          </section>
          ...
          <div style={grid}>
            <Info title="Project Information"><Row label="Project ID" value={project.project_id} /><Row label="Project Code" value={project.code} /><Row label="Contract No." value={project.contract_number} /><Row label="Client" value={project.client_name} /></Info>
            <Info title="Project Address"><Row label="Address" value={[project.address_line, project.address_number].filter(Boolean).join(', ')} /><Row label="Neighborhood" value={project.neighborhood} /><Row label="City / State" value={[project.city, project.state_region].filter(Boolean).join(', ')} /><Row label="ZIP / Country" value={[project.postal_code, project.country_code].filter(Boolean).join(' · ')} /></Info>
            <Info title="Contract & Financials"><Row label="Contract Value" value={money(project.contract_value, project.currency_code || 'BRL')} /><Row label="Material Value" value={project.material_included ? money(project.material_value, project.currency_code || 'BRL') : 'Not included'} /><Row label="Retainage" value={project.has_retainage ? `${project.retainage_percent ?? '—'}% · ${money(project.retainage_value, project.currency_code || 'BRL')}` : 'No'} /><Row label="Payment" value={project.has_retainage ? `${project.retainage_payment_days ?? '—'} days · ${dateValue(project.probable_retainage_payment_date)}` : '—'} /></Info>
            <Info title="Schedule"><Row label="Planned Start" value={dateValue(project.planned_start_date)} /><Row label="Contractual Term" value={project.contractual_term_days ? `${project.contractual_term_days} days` : '—'} /><Row label="Planned End" value={dateValue(project.planned_finish_date)} /><Row label="Status" value={project.status} /></Info>
          </div>

          <section style={successPanel}><div><div style={successTitle}>Success Criteria</div><div style={successText}>{project.success_criteria || 'Success criteria not defined yet.'}</div></div><div style={sourceTruth}>One Project · One Source of Truth</div></section>
        </>}
      </section>
    </main>
  )
...
function Info({ title, children }) { return <section style={panel}><h2 style={cardTitle}>{title}</h2>{children}</section> }
function Row({ label, value }) {
  const display = value === null || value === undefined || value === '' ? '—' : value
  return <div style={row}><span style={rowLabel}>{label}</span><strong style={rowValue}>{display}</strong></div>
}
...
const deleteButton = { border: '1px solid #dc4545', background: '#fff5f5', color: '#b42323', borderRadius: 9, padding: '9px 13px', fontWeight: 800, fontSize: 12, whiteSpace: 'nowrap' }
const errorBanner = { marginTop: 10, padding: '10px 14px', border: '1px solid #efb0b0', background: '#fff3f3', color: '#a61b1b', borderRadius: 9, fontSize: 12.5, fontWeight: 700 }
