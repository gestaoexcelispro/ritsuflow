  ...
  <section style={content}>
   ...
   {!loading&&project&&<div style={workspace}>
    <section style={hero}>
     ...
     <button style={editButton}>✎ Edit Project</button><button onClick={deleteProject} disabled={deleting} style={deleteButton}>▢ {deleting?'Deleting...':'Delete Project'}</button>
    </section>
    ...
    <div style={bodyGrid}>
     <div style={mainColumn}>
      ...
      <section style={successPanel}><div><div style={successTitle}>⚑ &nbsp; Success Criteria</div><div style={successText}>{project.success_criteria||'Success criteria not defined yet.'}</div></div><button style={compactButton}>✎ Edit</button></section>
     ...
     <aside style={sideColumn}><section style={sideCard}><div style={sideTitle}>▰ &nbsp; Project Image</div><div style={imagePlaceholder}><div style={{fontSize:28}}>▥</div><div>Project image</div></div><button style={sideAction}>↥ Add Project Image</button></section><section style={sideCard}><div style={sideTitle}>♥ &nbsp; Location Map</div><div style={mapPlaceholder}><div style={{fontSize:24}}>●</div><strong>{project.city||'Project location'}</strong><span>{project.neighborhood||''}</span></div><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent([project.address_line,project.address_number,project.city,project.state_region,project.postal_code,project.country_code].filter(Boolean).join(', '))}`} target="_blank" rel="noreferrer" style={mapLink}>● Open in Google Maps ↗</a></section></aside>
    </div>
   </div>}
  </section>
 </main>
...
const mapPlaceholder={flex:1,minHeight:80,borderRadius:6,background:'linear-gradient(45deg,#eef2ed 25%,#f7f7f3 25%,#f7f7f3 50%,#eef2ed 50%,#eef2ed 75%,#f7f7f3 75%)',backgroundSize:'22px 22px',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',color:'#d73b2e',gap:3,fontSize:11};const mapLink={display:'block',marginTop:6,color:'#1971c7',fontWeight:800,textDecoration:'none',fontSize:10.5}
