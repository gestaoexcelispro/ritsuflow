...
export default function StandaloneLocationWorkspace({ projectId, projectName, projectCode, userId, initialLocations = [], scopeItems = [], allocations = [] }) {
  ...
  const [draftAllocations, setDraftAllocations] = useState(() => {
    const draft = {}
    allocations.forEach((item) => { draft[`${item.service_id}:${item.location_id}`] = item.quantity == null ? '' : String(item.quantity) })
    return draft
  })
  ...
  const selectedScope = useMemo(() => {
    ...
    allocationRows.filter((item) => item.location_id === selectedId).forEach((item) => {
      byService.set(item.service_id, (byService.get(item.service_id) || 0) + number(item.quantity))
    })
  ...
  const serviceTotals = useMemo(() => {
    const totals = new Map(scopeItems.map((item) => [item.id, 0]))
    allocationRows.forEach((item) => totals.set(item.service_id, (totals.get(item.service_id) || 0) + number(item.quantity)))
    return totals
  }, [allocationRows, scopeItems])

  const visibleScopeItems = useMemo(() => {
    const query = scopeSearch.trim().toLowerCase()
    return scopeItems.filter((item) => {
      if (showAllocatedOnly && (serviceTotals.get(item.id) || 0) <= 0) return false
      if (!query) return true
      return `${item.service_code || ''} ${item.service_name || ''} ${item.unit || ''}`.toLowerCase().includes(query)
    })
  }, [scopeItems, scopeSearch, showAllocatedOnly, serviceTotals])
  ...
  const draftSelectedTotal = useMemo(() => {
    if (!selectedServiceId) return 0
    return locations.reduce((sum, location) => sum + number(draftAllocations[`${selectedServiceId}:${location.id}`]), 0)
  }, [draftAllocations, locations, selectedServiceId])
  ...
  function descendants(id) {
    ...
    while (queue.length) { const current = queue.shift(); (childrenMap.get(current) || []).forEach((child) => { result.push(child.id); queue.push(child.id) }) }
    return result
  }
  function matchesBranch(item) {
    ...
    return descendants(item.id).some((id) => { const child = locationMap.get(id); return child && `${child.name} ${child.location_type} ${child.environment_type || ''}`.toLowerCase().includes(normalizedSearch) })
  }
  ...
  async function saveLocation(event) {
    ...
    try {
      if (existing) {
        ...
        await history('location_updated', 'Location updated', `${existing.name} was updated in the Location Breakdown Structure`, data.id, { location_name: data.name, changes })
    ...
    setSelectedId(data.id); setSaving(false); setModalOpen(false); setForm(emptyForm); router.refresh()
  }
  ...
  function renderAllocationNode(item, depth = 0) {
    ...
    const isCollapsed = allocationCollapsed.has(item.id)
    const isProduction = !nonProductionTypes.has(item.location_type)
    const key = `${selectedServiceId}:${item.id}`
    return <div key={item.id}>
      <div className={`${styles.allocationTreeRow} ${isProduction ? styles.allocationProductionRow : ''}`} style={{ '--depth': depth }}>
        <button type="button" className={styles.chevron} onClick={() => children.length && toggleAllocation(item.id)}>{children.length ? (isCollapsed ? '›' : '⌄') : ''}</button>
        <span className={`${styles.nodeIcon} ${styles[`nodeIcon_${item.location_type}`] || ''}`}>{typeIcon(item.location_type)}</span>
        <div className={styles.allocationLocationName}><strong>{item.name}</strong><span>{typeLabel(item.location_type)}</span></div>
        {isProduction ? <div className={styles.quantityField}><input type="number" min="0" step="any" value={draftAllocations[key] ?? ''} onChange={(event) => { setAllocationMessage(''); setDraftAllocations((current) => ({ ...current, [key]: event.target.value })) }} placeholder="0.00" /><span>{selectedService?.unit || ''}</span></div> : <span className={styles.groupLabel}>Group</span>}
      </div>
  ...
  function clearSelectedAllocation() {
    ...
    setDraftAllocations((current) => {
      const next = { ...current }
      locations.forEach((location) => { next[`${selectedServiceId}:${location.id}`] = '' })
      return next
    })
    setAllocationMessage('Allocation cleared locally. Save to apply the change.')
  }

  async function saveAllocation() {
    if (!selectedService) return
    if (overAllocated) { setAllocationMessage('Allocated quantity cannot exceed the production scope quantity.'); return }
    setAllocationSaving(true); setAllocationMessage('')
    const existingForService = allocationRows.filter((item) => item.service_id === selectedService.id)
    const existingByLocation = new Map(existingForService.map((item) => [item.location_id, item]))
    ...
    const desiredIds = new Set(desired.map((item) => item.location.id))
    const deleteIds = existingForService.filter((item) => !desiredIds.has(item.location_id)).map((item) => item.id)

    if (deleteIds.length) {
      const { error: deleteError } = await supabase.from('location_service_quantities').delete().in('id', deleteIds).eq('project_id', projectId)
      if (deleteError) { setAllocationMessage(deleteError.message); setAllocationSaving(false); return }
    }
    for (const item of desired) {
      const existing = existingByLocation.get(item.location.id)
      if (existing) {
        const { error: updateError } = await supabase.from('location_service_quantities').update({ quantity: item.quantity }).eq('id', existing.id).eq('project_id', projectId)
        if (updateError) { setAllocationMessage(updateError.message); setAllocationSaving(false); return }
      } else {
        const { error: insertError } = await supabase.from('location_service_quantities').insert({ project_id: projectId, location_id: item.location.id, service_id: selectedService.id, quantity: item.quantity, created_by: userId })
  ...
  return <div className={styles.workspace}>
    ...
    <section className={styles.toolbar}>
      ...
      {activeTab === 'locations' ? <div className={styles.toolbarActions}><div className={styles.searchWrap}><span>⌕</span><input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Search locations..." /></div><button type="button" className={styles.addButton} onClick={() => openAdd('', 'floor')}>＋ Add location</button></div> : <div className={styles.allocationToolbarNote}>Allocate production quantities to the lowest meaningful production location.</div>}
    </section>
    ...
    {activeTab === 'locations' ? <section className={styles.mainGrid}>
      ...
      <div className={styles.detailPanel}>{selected ? <div className={styles.selectedDetail}>
        ...
        {selectedScope.length ? <div className={styles.scopeList}>{selectedScope.map((item) => <div key={item.id}><strong>{item.service_name}</strong><span>{formatQuantity(item.allocatedQuantity)} {item.unit || ''}</span></div>)}</div> : <div className={styles.noScope}>No scope is linked to this location yet.</div>}
    ...
    </section> : <section className={styles.allocationWorkspace}>
      <aside className={styles.scopePanel}>
        <div className={styles.scopePanelHeader}><div><span>PRODUCTION SCOPE</span><strong>{scopeItems.length} activities</strong></div><label className={styles.allocatedToggle}><input type="checkbox" checked={showAllocatedOnly} onChange={(event) => setShowAllocatedOnly(event.target.checked)} /> Allocated only</label></div>
        <div className={styles.scopeSearch}><span>⌕</span><input value={scopeSearch} onChange={(event) => setScopeSearch(event.target.value)} placeholder="Search production scope..." /></div>
        <div className={styles.serviceList}>{visibleScopeItems.length ? visibleScopeItems.map((item) => {
          const total = number(item.scope_quantity); const allocated = serviceTotals.get(item.id) || 0; const percent = total > 0 ? Math.min(100, (allocated / total) * 100) : 0; const status = allocated <= 0 ? 'Not allocated' : total > 0 && allocated >= total - 0.000001 ? 'Fully allocated' : 'Partially allocated'
          return <button type="button" key={item.id} className={`${styles.serviceCard} ${selectedServiceId === item.id ? styles.serviceCardActive : ''}`} onClick={() => { setSelectedServiceId(item.id); setAllocationMessage('') }}>
            <div className={styles.serviceCardTop}><span>{item.service_code || 'Scope'}</span><em className={status === 'Fully allocated' ? styles.statusComplete : status === 'Partially allocated' ? styles.statusPartial : ''}>{status === 'Fully allocated' ? '✓ ' : ''}{status}</em></div>
            <strong>{item.service_name}</strong><div className={styles.serviceQuantity}>{formatQuantity(total)} {item.unit || ''}</div><div className={styles.miniProgress}><i style={{ width: `${percent}%` }} /></div><small>{formatQuantity(allocated)} allocated</small>
          </button>
        }) : <div className={styles.emptyServices}>No production scope matches this filter.</div>}</div>
      </aside>

      <div className={styles.allocationPanel}>{selectedService ? <>
        <div className={styles.allocationHeader}>
          <div><span>SCOPE ALLOCATION</span><h2>{selectedService.service_name}</h2><p>{selectedService.service_code || 'Production activity'} · {selectedService.unit || 'No unit'}</p></div>
          <div className={styles.allocationStats}><div><span>Scope Qty</span><strong>{formatQuantity(selectedServiceTotal)} {selectedService.unit || ''}</strong></div><div><span>Allocated</span><strong>{formatQuantity(draftSelectedTotal)} {selectedService.unit || ''}</strong></div><div className={overAllocated ? styles.statDanger : ''}><span>Remaining</span><strong>{formatQuantity(remaining)} {selectedService.unit || ''}</strong></div></div>
        </div>
        <div className={styles.progressBlock}><div><strong>{overAllocated ? 'Over allocated' : `${Math.min(100, allocationPercent).toFixed(0)}% allocated`}</strong><span>{formatQuantity(draftSelectedTotal)} / {formatQuantity(selectedServiceTotal)} {selectedService.unit || ''}</span></div><div className={`${styles.progressTrack} ${overAllocated ? styles.progressDanger : ''}`}><i style={{ width: `${Math.min(100, allocationPercent)}%` }} /></div></div>
        <div className={styles.allocationTreeHeader}><span>Location Breakdown</span><span>Allocated quantity</span></div>
        <div className={styles.allocationTree}>{roots.length ? roots.map((item) => renderAllocationNode(item, 0)) : <div className={styles.emptyServices}>Create the Location Breakdown Structure before allocating scope.</div>}</div>
        <div className={styles.allocationFooter}>{allocationMessage ? <span className={overAllocated ? styles.messageDanger : styles.message}>{allocationMessage}</span> : <span className={styles.allocationHint}>Quantities are stored against the existing production service and location records.</span>}<div><button type="button" className={styles.clearButton} disabled={allocationSaving} onClick={clearSelectedAllocation}>Clear</button><button type="button" className={styles.saveAllocationButton} disabled={allocationSaving || overAllocated} onClick={saveAllocation}>{allocationSaving ? 'Saving...' : 'Save Allocation'}</button></div></div>
      </> : <div className={styles.emptyAllocation}><strong>Select a production scope item</strong><span>Choose an activity on the left to distribute its quantity through the Location Breakdown Structure.</span></div>}</div>
    </section>}
