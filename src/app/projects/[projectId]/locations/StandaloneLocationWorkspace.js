...
function typeLabel(value) {
  return TYPES.find((item) => item.value === value)?.label || 'Location'
}
...
export default function StandaloneLocationWorkspace({ projectId, projectName, projectCode, userId, initialLocations = [], scopeItems = [], allocations = [] }) {
  ...
  const buildings = locations.filter((item) => item.location_type === 'building').length
  ...
  function nextSequence() {
    return locations.reduce((max, item) => Math.max(max, Number(item.sequence_number) || 0), 0) + 1
  }

  function openAdd(parentId = '', locationType = 'floor') {
    setForm({ ...emptyForm, parent_id: parentId, location_type: locationType })
    setError('')
    setModalOpen(true)
  }

  function openEdit(item) {
    setForm({ id: item.id, location_type: item.location_type, name: item.name || '', parent_id: item.parent_id || '', environment_type: item.environment_type || '' })
    setError('')
    setModalOpen(true)
  }
  ...
  async function saveLocation(event) {
    ...
    const payload = {
      ...
      environment_type: form.environment_type.trim() || null,
      sequence_number: existing?.sequence_number ?? nextSequence(),
    }

    let query
    if (form.id) {
      query = supabase.from('locations').update(payload).eq('id', form.id).eq('project_id', projectId)
    } else {
      query = supabase.from('locations').insert({ ...payload, created_by: userId })
    }
    ...
    if (saveError) {
      setError(saveError.message || 'The location could not be saved.')
      setSaving(false)
      return
    }
  ...
  async function removeLocation(item) {
    ...
    if (childIds.length) {
      ...
      if (rpcError || data?.deleted !== true) {
        setError(rpcError?.message || data?.message || 'The location could not be deleted.')
        setSaving(false)
        return
      }
    } else {
  ...
  function suggestedChildType(type) {
    if (type === 'building') return 'floor'
    if (type === 'floor') return 'zone'
    return 'area'
  }
    ...
    return (
      <div key={item.id}>
        <div className={`${styles.treeRow} ${active ? styles.treeRowActive : ''}`} style={{ '--depth': depth }} onClick={() => setSelectedId(item.id)}>
          <button type="button" className={styles.chevron} onClick={(event) => { event.stopPropagation(); if (children.length) toggle(item.id) }} aria-label={isCollapsed ? 'Expand location' : 'Collapse location'}>
    ...
    <div className={styles.workspace}>
      <section className={styles.projectSummary}>
        <div className={styles.projectIdentity}>
          ...
          <div>
            <span>{projectCode || 'Project'}</span>
            <strong>{projectName}</strong>
          </div>
        </div>
        <Metric icon="⌖" value={locations.length} label="Total Locations" />
        <Metric icon="▤" value={divisions} label="Divisions" />
        <Metric icon="▦" value={zones} label="Zones" />
        <Metric icon="◇" value={production} label="Production locations" />
      </section>

      <section className={styles.toolbar}>
        <div className={styles.tabs}>
          <button type="button" className={`${styles.tab} ${styles.tabActive}`}>☷ <span>Location Breakdown</span></button>
        ...
        <div className={styles.toolbarActions}>
          <div className={styles.searchWrap}><span>⌕</span><input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Search locations..." /></div>
          <button type="button" className={styles.addButton} onClick={() => openAdd('', 'floor')}>＋ Add location</button>
        </div>
      </section>
      ...
      <section className={styles.mainGrid}>
        <div className={styles.treePanel}>
          ...
          <div className={styles.projectRootRow}>
            <span className={styles.chevron}>⌄</span><span className={styles.projectRootIcon}>▦</span><strong>{projectName}</strong><span className={styles.typeBadge}>Project root</span><button type="button" className={styles.rootAdd} onClick={() => openAdd('', 'floor')}>＋</button>
          </div>
          <div className={styles.treeBody}>
            {roots.length ? roots.map((item) => renderNode(item, 0)) : (
              <div className={styles.treeEmpty}><strong>No locations yet</strong><span>Start with the first Division / Floor.</span><button type="button" onClick={() => openAdd('', 'floor')}>＋ Add division</button></div>
            )}
          </div>
        </div>
        ...
        <div className={styles.detailPanel}>
          {selected ? (
            ...
            <div className={styles.emptyDetail}>
              <div className={styles.illustration}><Image src="/lbs-icon.png" alt="Location Breakdown Structure" fill sizes="(max-width: 1200px) 45vw, 620px" style={{ objectFit: 'contain' }} priority /></div>
              <h2>Select a location</h2>
              <p>Choose a location from the list to view details, linked scope and mapped elements.</p>
            </div>
          )}
        </div>
      </section>
      ...
      {modalOpen ? (
        <div className={styles.modalBackdrop} onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) setModalOpen(false) }}>
...
function Metric({ icon, value, label }) {
  return <div className={styles.metric}><span className={styles.metricIcon}>{icon}</span><div><strong>{value}</strong><span>{label}</span></div></div>
}
