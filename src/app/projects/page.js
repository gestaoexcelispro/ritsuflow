    ...
    <div className={styles.tableWrap}>
     <table><thead><tr><th>Project</th><th>Location</th><th>Project Manager</th><th>Phase</th><th>Workers</th><th>Operations</th><th>Today’s Progress</th><th>Open Issues</th><th>Status</th><th>Last Update</th><th></th></tr></thead>
      <tbody>{projects.map(p=><tr key={p.code}>
       ...
       <td>18 Sep · {p.update}</td><td><Link className={styles.openProject} href={`/projects/${p.code.toLowerCase()}`}>Open Project →</Link></td>
      </tr>)}</tbody>
     </table>
    </div>
    <footer className={styles.tableFooter}><span>Showing 8 active projects</span><span>One shared project source for PreCon and FieldOp</span></footer>
   </section>
  </div>
 </main>
}
