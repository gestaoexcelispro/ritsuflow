  ...
  return <main className={styles.shell}>
    ...
    <section className={styles.main}><header className={styles.topbar}><div><div className={styles.crumb}><Link href="/fieldop/projects">Projects</Link><span>/</span>{loading ? 'Loading...' : projectName}</div><strong>Daily Report Setup</strong></div><div className={styles.search}>⌕ <span>Search project setup...</span><kbd>Ctrl K</kbd></div><div className={styles.user}><b>EF</b><div><strong>Eduardo Freitas</strong><span>Operations Manager</span></div></div></header>
      <div className={styles.content}><section className={styles.projectHeader}><div className={styles.projectIcon}>{projectName.charAt(0).toUpperCase()}</div><div><h1>{projectName}</h1><p>{project?.project_id || '—'} · {project?.client || project?.client_name || '—'} · {location}</p></div><span className={styles.status}>○ FieldOp setup in progress</span></section><div className={styles.tabs}>{tabs.map(tab => <button key={tab} className={activeTab === tab ? styles.tabActive : ''} onClick={() => setActiveTab(tab)}>{tab}</button>)}</div>
        ...
        {activeTab === 'Locations' && <section className={styles.workspace}><div><h2>Locations</h2><p>FieldOp will use the canonical project Location Structure. Location selection will be configured here.</p></div><div className={styles.empty}><b>Location setup is next.</b><span>No duplicate location hierarchy will be created in FieldOp.</span></div></section>}{activeTab === 'Workforce' && <section className={styles.workspace}><div><h2>Workforce</h2><p>Configure project crews and workers available for Daily Reports and Operations.</p></div><div className={styles.empty}><b>Workforce setup is not configured yet.</b></div></section>}{activeTab === 'Daily Report Settings' && <section className={styles.workspace}><div><h2>Daily Report Settings</h2><p>Configure project-specific Daily Report behavior without changing the canonical project record.</p></div><div className={styles.empty}><b>Daily Report settings are not configured yet.</b></div></section>}</div></section>
